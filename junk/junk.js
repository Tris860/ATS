...
const fetchUserEmailAndSettings = async () => {
  ...
  try {
    ...
    if (data.success) {
      ...
      WS_URL = `wss://webserver-ft8c.onrender.com/ws/browser?email=${encodeURIComponent(
        USER_EMAIL
      )}`;
...
toggleHardSwitch.onchange = async (e) => {
  ...
  try {
    ...
    await showCustomModal(
      "Network Error",
      error +
        "Could not toggle hard switch. Please check your network connection."
    );
  }
};
...
function friendlyStatus(status) {
  ...
  if (s === "CONNECTED" || s === "READY" || s === "AVAILABLE")
...
function softenAck(resp) {
  ...
  // Success
  if (
    r.includes('"status":"ok"') ||
    (r.toLowerCase().includes("command") && r.toLowerCase().includes("sent"))
  ) {
  ...
  if (
    r.toLowerCase().includes("failed to connect") ||
...
function connectWebSocket() {
  ...
  socket.onmessage = async (event) => {
    ...
    if (type === "device_binding") {
      ...
      notifyUser(
        "Device bound",
        deviceName ? `Connected to ${deviceName}.` : "No device bound."
      );
      return;
    }
    ...
    if (type === "device_status") {
      ...
      const ready = ["CONNECTED", "READY", "AVAILABLE"].includes(
...
async function SendSignal(command) {
  ...
  try {
    ...
    showCustomModal(
      "Action failed",
      "Unable to send your command. Please try again."
    );
  }
}
